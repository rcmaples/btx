...
export function useCart() {
  ...
  const addToCart = useCallback(async (selection: PurchaseSelection, quantity: number = 1) => {
    setIsLoading(true)
    setError(null)
  ...
  const itemCount = mounted ? cart.lineItems.reduce((sum, item) => sum + item.quantity, 0) : 0
