...
export function SubscriptionConfigurator({
  ...
  const handleAddSubscriptionToCart = async () => {
    ...
    try {
      ...
      const subscription = subscriptionService.createSubscription(selectedPurchaseOption, cadence)
  ...
  const recurringPrice = selectedPurchaseOption ? selectedPurchaseOption.priceInCents : 0
    ...
    <div className="mt-xl pt-xl border-t-2 border-border">
      ...
      <div>
        ...
        <p className="text-xs text-text-muted mt-sm text-center">
          Note: Subscription management (pause, modify, cancel) is not available in this demo
          version
        </p>
      </div>
    </div>
